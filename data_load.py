...
def decide_config(root, dataset):
    ...
    if dataset == 'cora_full':
        ...
        dataset = "BlogCatalog"
        root = osp.join(root, "pyg", 'BlogCatalog')
        params = {"kwargs": {"root": root},
                  "name": dataset, "class": BlogCatalog, "src": "pyg"}
        ...
        raise Exception(
            f"Unknown dataset name {dataset}, name has to be one of the following 'cora_full', 'email', 'blog', 'wiki', 'amz_cloth', 'amz_eletronics'")
    return params
...
class DataGPN(InMemoryDataset):
    ...
    def process(self):
        ...
        adj = utils.normalize_sp_adj(adj + sp.eye(adj.shape[0]))
        features = torch.FloatTensor(features)
