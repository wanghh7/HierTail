...
class origin():
    ...
    def training(self):
        ...
        if self.args.lt_setting == 1: # natural
            ...
            os.makedirs(f'./results/baseline/manual/{self.args.dataset}/{self.args.im_class_num}/{self.args.lt_setting}', exist_ok=True)
            text = open(f'./results/baseline/manual/{self.args.dataset}/{self.args.im_class_num}/{self.args.lt_setting}/{self.args.embedder}.txt', 'w')
        ...
        for seed in range(self.args.rnd, self.args.rnd+self.args.num_seed):
            ...
            for epoch in range(self.args.ep):
                ...
                st += "[Val] ACC: {:.1f}, Macro-F1: {:.3f}, G-Means: {:.3f}, bACC: {:.3f}|| ".format(acc_val, macro_F_val, gmeans_val, bacc_val)
                st += "[Test] ACC: {:.3f}, Macro-F1: {:.3f}, G-Means: {:.3f}, bACC: {:.3f}\n".format(acc_test, macro_F_test, gmeans_test, bacc_test)
                st += "  [*Best Test Result*][Epoch {}] ACC: {:.3f}, Macro-F1: {:.3f}, G-Means: {:.3f}, bACC: {:.3f}".format(max_idx, best_test_result[0], best_test_result[1], best_test_result[2], best_test_result[3])
                ...
                if (epoch - max_idx > self.args.ep_early) or (epoch+1 == self.args.ep):
                    ...
                    acc_list, macro_F_list, gmean_list, bacc_list = utils.performance_per_class(output[self.idx_test],
                                                                                                self.labels[
                                                                                                    self.idx_test],
                                                                                                pre='test')
                    print("[Best Test Result] ACC: {:.3f}, Macro-F1: {:.3f}, G-Means: {:.3f}, bACC: {:.3f}".format(best_test_result[0], best_test_result[1], best_test_result[2], best_test_result[3]),file=text)
        ...
        print('[Averaged result] ACC: {:.3f}+{:.3f}, Macro-F: {:.3f}+{:.3f}, G-Means: {:.3f}+{:.3f}, bACC: {:.3f}+{:.3f}'.format(np.mean(acc), np.std(acc), np.mean(f1), np.std(f1), np.mean(gm), np.std(gm), np.mean(bacc), np.std(bacc)))
        print(file=text)
        print('ACC Macro-F G-Means bACC', file=text)
        print('{:.3f}+{:.3f} {:.3f}+{:.3f} {:.3f}+{:.3f} {:.3f}+{:.3f}'.format(np.mean(acc), np.std(acc), np.mean(f1), np.std(f1), np.mean(gm), np.std(gm), np.mean(bacc), np.std(bacc)), file=text)
        print(file=text)
